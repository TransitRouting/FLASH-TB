...
class WriteLayoutGraphToGraphML : public ParameterizedCommand {
 public:
  WriteLayoutGraphToGraphML(BasicShell& shell)
      : ParameterizedCommand(shell, "writeLayoutGraphToGraphML",
                             "Writes the Layout Graph into a GraphML file.") {
    addParameter("RAPTOR Binary");
...
class WriteTripBasedToGraphML : public ParameterizedCommand {
 public:
  WriteTripBasedToGraphML(BasicShell& shell)
      : ParameterizedCommand(
            shell, "writeTripBasedToGraphML",
            "Writes the StopEvent Graph into a GraphML file.") {
    addParameter("Trip Based Binary");
  ...
  virtual void execute() noexcept {
    const std::string networkFile = getParameter("Trip Based Binary");
    const std::string outputFileStopEvent =
        getParameter("Output file (StopEvent Graph)");
    const std::string outputFileLayout =
        getParameter("Output file (Layout Graph)");

    TripBased::Data network = TripBased::Data(networkFile);
    network.raptorData.createGraphForMETIS(
        RAPTOR::TRIP_WEIGHTED | RAPTOR::TRANSFER_WEIGHTED, true);
