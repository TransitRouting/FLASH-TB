...
class ComputeTransitiveEventToEventShortcuts : public ParameterizedCommand {
 public:
  ComputeTransitiveEventToEventShortcuts(BasicShell &shell)
      : ParameterizedCommand(
            shell, "computeTransitiveEventToEventShortcuts",
            "Computes transitive event-to-event transfer shortcuts using ULTRA "
            "and saves the resulting network in Trip-Based format.") {
  ...
  virtual void execute() noexcept {
    ...
    std::cout
        << "Computing transitive event-to-event ULTRA shortcuts (parallel with "
...
class ComputeArcFlagTB : public ParameterizedCommand {
 public:
  ...
  virtual void execute() noexcept {
    ...
    TripBased::ARCFlagTBBuilder arcFlagComputer(trip, getNumberOfThreads(),
                                                pinMultiplier);
...
class ApplyPartitionToTripBased : public ParameterizedCommand {
 public:
  ApplyPartitionToTripBased(BasicShell &shell)
      : ParameterizedCommand(shell, "applyPartitionToTripBased",
                             "Applies the given partition to the stops of the "
                             "Trip-Based input and saves it.") {
...
class ComputeArcFlagTBRAPTOR : public ParameterizedCommand {
 public:
  ComputeArcFlagTBRAPTOR(BasicShell &shell)
      : ParameterizedCommand(
            shell, "computeArcFlagTBRAPTOR",
            "Computes Arc-Flags for the given RAPTOR Data and the "
            "given Partition File (from KaHIP) using the RangeRAPTOR with "
            "ULTRA conditions.") {
    addParameter("Input file (RAPTOR Data)");
    addParameter("Output file (Trip Data)");
    addParameter("Input file (Partition File)", "None");
  ...
  virtual void execute() noexcept {
    ...
    TripBased::ComputeARCFlagsProfileRAPTOR arcFlagComputer(
        raptor, trip, getNumberOfThreads(), pinMultiplier);
    arcFlagComputer.computeARCFlags(verbose);
...
class CreateLayoutGraph : public ParameterizedCommand {
 public:
  ...
  virtual void execute() noexcept {
    ...
    raptor.createGraphForMETIS(
        RAPTOR::TRIP_WEIGHTED | RAPTOR::TRANSFER_WEIGHTED, true);
