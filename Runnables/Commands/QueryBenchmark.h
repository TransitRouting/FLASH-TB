 ...
 public:
  ...
  virtual void execute() noexcept {
    ...
    for (const StopQuery &query : queries) {
      ...
      numJourneys += journeys.back().size();
    }
    algorithm.getProfiler().printStatistics();
    std::cout << "Avg. journeys: " << String::prettyDouble(numJourneys / n)
              << std::endl;

    IO::serialize("tb.queries", queries);
    IO::serialize("tb.journeys", journeys);
  }
};
 ...
 public:
  TestTransitiveArcTripBasedQueries(BasicShell &shell)
      : ParameterizedCommand(shell, "testTransitiveArcTripBasedQueries",
                             "Compares results from the Arc-Flag TB Query vs "
                             "results from the TB Query.") {
    addParameter("Trip-Based input file");
  ...
  virtual void execute() noexcept {
    ...
    IO::deserialize(getParameter("TB Journeys (binary) file"),
                    originalTBJourneys);
    ...
    TripBased::ARCTransitiveQuery<TripBased::AggregateProfiler> algorithm(
        tripBasedData);
    ...
    for (size_t i(0); i < queries.size(); ++i) {
      ...
      for (size_t j(0); j < originalTBJourneys[i].size(); ++j) {
        for (size_t k(0); k < arcTBJourneys[i].size(); ++k) {
          if (countTrips(originalTBJourneys[i][j]) ==
                  countTrips(arcTBJourneys[i][k]) &&
              originalTBJourneys[i][j].back().arrivalTime >=
                  arcTBJourneys[i][k].back().arrivalTime) {
      ...
      if (foundRightJourneys != originalTBJourneys[i].size()) {
        ++absolutWrong;
        std::cout << "Query from " << queries[i].source << " to "
                  << queries[i].target << " @ "
                  << String::secToString(queries[i].departureTime) << std::endl;
        std::cout << "TB found the following set:\n";
        for (auto j : originalTBJourneys[i]) {
          for (auto leg : j) std::cout << leg << std::endl;
        }
        std::cout << "ArcTB found the following set:\n";
        for (auto j : arcTBJourneys[i]) {
          for (auto leg : j) std::cout << leg << std::endl;
        }
      }
    }
    std::cout << "Query errors:\nOptimal Journey found by TB:\t"
              << totalRightJourneys << "\nOptimal Journeys found by Arc TB:\t"
              << totalRightJourneysArc << "\nThis represents:\t"
              << ((float)totalRightJourneysArc / (float)totalRightJourneys) *
                     100
              << " %\n";
    std::cout << "Absolute Wrong Queries:\t" << absolutWrong
              << "\nThis represents:\t"
              << ((float)absolutWrong / queries.size()) * 100 << " %\n";
  }
};
 ...
 public:
  ...
  virtual void execute() noexcept {
    ...
    if (getParameter<bool>("Compressed?")) {
      TripBased::ARCTransitiveQueryComp<TripBased::AggregateProfiler> algorithm(
          tripBasedData, inputFile);
      for (const StopQuery &query : queries) {
        algorithm.run(query.source, query.departureTime, query.target);
        numJourneys += algorithm.getJourneys().size();
      }
...
class RunTransitiveProfileArcTripBasedQueries : public ParameterizedCommand {
 public:
  RunTransitiveProfileArcTripBasedQueries(BasicShell &shell)
      : ParameterizedCommand(
            shell, "runTransitiveProfileArcTripBasedQueries",
            "Runs the given number of random transitive Arc-Flag TB queries "
            "with a time range of [0, 24 hours).") {
    addParameter("Trip-Based input file");
    addParameter("Number of queries");
  }
  ...
  virtual void execute() noexcept {
    ...
    TripBased::Data tripBasedData(inputFile);
    tripBasedData.printInfo();
    TripBased::ARCProfileQuery<TripBased::AggregateProfiler> algorithm(
        tripBasedData);
