...
public:
    ...
    virtual void execute() noexcept {
        ...
        for (size_t i(0); i < queries.size(); ++i) {
            ...
            for (size_t j(0); j < originalTBJourneys[i].size(); ++j) {
                for (size_t k(0); k < arcTBJourneys[i].size(); ++k) {
                    if (countTrips(originalTBJourneys[i][j]) == countTrips(arcTBJourneys[i][k])
                        && originalTBJourneys[i][j].back().arrivalTime >= arcTBJourneys[i][k].back().arrivalTime) {
            ...
            if (foundRightJourneys != originalTBJourneys[i].size()) {
                ++absolutWrong;
                std::cout << "Query from " << queries[i].source << " to " << queries[i].target << " @ "
        ...
        std::cout << "Query errors:\nOptimal Journey found by TB:\t" << totalRightJourneys
                  ...
                  << ((float)totalRightJourneysArc / (float)totalRightJourneys) * 100 << " %\n";
...
public:
    ...
    virtual void execute() noexcept {
        ...
        if (getParameter<bool>("Compressed?")) {
            TripBased::ARCTransitiveQueryComp<TripBased::AggregateProfiler> algorithm(tripBasedData, inputFile);
            for (const StopQuery& query : queries) {
                algorithm.run(query.source, query.departureTime, query.target);
                numJourneys += algorithm.getJourneys().size();
            }
            ...
            for (const double tripSlack : tripSlacks) {
                ...
                for (const VertexQuery& query : queries) {
                    ...
                    if (bestAnchorLabel.walkingDistance == 0) {
                        ...
                        transferTimeSavings.emplace_back((bestAnchorLabel.walkingDistance - bestLabel.walkingDistance)
