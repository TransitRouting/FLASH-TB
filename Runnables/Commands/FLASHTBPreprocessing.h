...
public:
    ComputeTransitiveEventToEventShortcuts(BasicShell& shell)
        : ParameterizedCommand(shell, "computeTransitiveEventToEventShortcuts",
    ...
    virtual void execute() noexcept {
        ...
        std::cout << "Computing transitive event-to-event ULTRA shortcuts (parallel with " << numberOfThreads
                  << " threads)." << std::endl;
...
public:
    ComputeArcFlagTB(BasicShell& shell)
        : ParameterizedCommand(shell, "computeArcFlagTB", "Computes Arc-Flags for the given TB Data!") {
...
public:
    ComputePageRank(BasicShell& shell)
        : ParameterizedCommand(shell, "computePageRank", "Computes the pagerank values for the given layout graph.") {
        addParameter("Input file");
    }
