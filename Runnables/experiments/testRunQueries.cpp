...
int main(int argc, char* argv[]) {
  if (argc != 4) {
    std::cout << "Usage: ./runQueries [TB File] [Query File] [TB Original "
                 "Journey File]"
              << std::endl;
    return -1;
  }
  ...
  */
  ...
  for (int i(0); i < queries.size(); ++i) {
    ...
    for (auto& leg : originalJourneys[i]) std::cout << leg << std::endl;
    std::cout << "Arc-Flag TB:" << std::endl;
    for (auto& leg : algorithm.getJourneys()) std::cout << leg << std::endl;
  }
}
