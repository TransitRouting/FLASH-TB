...
struct Parent {
 public:
  ...
  Parent(size_t n, size_t numElements)
      : storage(n, std::vector<TYPE_TO_STORE>(numElements)){};
  ...
  void fill(size_t n, TYPE_TO_STORE defaultValue) noexcept {
    fill(n, defaultValue, 0, storage[n].size());
  }

  void fill(size_t n, TYPE_TO_STORE defaultValue, size_t from,
            size_t to) noexcept {
    assert(isValid(n));
    assert(from <= to);
  ...
  const TYPE_TO_STORE& getElement(size_t element,
                                  size_t numberOfElement) const noexcept {
    assert(isElementOfElement(element, numberOfElement));
    return storage[element][numberOfElement];
  }

  void setElement(size_t element, size_t numberOfElement,
                  TYPE_TO_STORE newElement) noexcept {
    assert(isElementOfElement(element, numberOfElement));
