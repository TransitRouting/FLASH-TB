...
public:
    ...
    inline bool isVertex(const size_t vertex) { return (vertex < numVertices); }

    void incorperateGraph() {
        toAdjLocal.assign(data.numberOfStopEvents() + 1, 0);
        toAdjTransfer.assign(data.numberOfStopEvents() + 1, 0);
        toLocalVertex.clear();
        toTransferVertex.clear() localFlags.clear() transfeFlags.clear() numVertices = data.numberOfStopEvents();
        numLocalEdges = 0;
        numTransferEdges = 0;
