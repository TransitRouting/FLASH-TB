    ...
    inline std::vector<std::vector<Intermediate::Trip>> fifoRoutes() const {
        ...
        while (left < trips.size() && left <= right) {
            ...
            AssertMsg(checkFlowGraph(flowGraph),
                      "Edges are missing, this is not the wanted representation of the flowgraph!");
            ...
            for (Dinic::CutEdge& cutEdge : cutEdges) {
                ...
                if (newRouteIdOfVertex[(cutEdge.from >> 1)] == -1) {
                    ...
                    routes.emplace_back(std::vector<Intermediate::Trip>{trips[left + (cutEdge.from >> 1)],
                                                                        trips[left + ((cutEdge.to - 1) >> 1)]});
                    AssertMsg((size_t)newRouteIdOfVertex[(cutEdge.from >> 1)] + 1 == routes.size(),
                              "Regrowing of routes did not work?");
                } else {
                    newRouteIdOfVertex[(cutEdge.to - 1) >> 1] = newRouteIdOfVertex[(cutEdge.from >> 1)];
                    AssertMsg((size_t)newRouteIdOfVertex[(cutEdge.to - 1) >> 1] < routes.size(),
                              "Index out of bounds!");
                    routes[newRouteIdOfVertex[(cutEdge.to - 1) >> 1]].emplace_back(
                        trips[left + ((cutEdge.to - 1) >> 1)]);
                }
            }
            for (size_t i(0); i < newRouteIdOfVertex.size(); ++i) {
                if (newRouteIdOfVertex[i] != -1) continue;
                newRouteIdOfVertex[i] = routes.size();
                routes.emplace_back(std::vector<Intermediate::Trip>{trips[left + i]});
            }
            AssertMsg(std::find(newRouteIdOfVertex.begin(), newRouteIdOfVertex.end(), -1) == newRouteIdOfVertex.end(),
