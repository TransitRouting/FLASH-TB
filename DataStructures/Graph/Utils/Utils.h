...
template <typename GRAPH>
inline std::vector<double> pagerank(const GRAPH& graph, const GRAPH& revGraph, double damping = 0.85,
                                    double epsilon = 1e-8) {
    ...
    while (true) {
        ...
        for (Vertex i : graph.vertices()) {
            ...
            for (Vertex j : revGraph.outgoingNeighbors(i)) {
                new_pr[i] += damping * pr[j] / (graph.outDegree(j));
            }
