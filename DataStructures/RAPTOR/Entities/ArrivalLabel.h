...
struct WalkingParetoLabel {
    ...
    WalkingParetoLabel(const int arrivalTime = never, const int walkingDistance = INFTY, const int numberOfTrips = -1)
        : arrivalTime(arrivalTime), walkingDistance(walkingDistance), numberOfTrips(numberOfTrips) {}

    template <typename LABEL>
    WalkingParetoLabel(const LABEL& label, const size_t numberOfTrips)
        : arrivalTime(label.arrivalTime), walkingDistance(label.walkingDistance), numberOfTrips(numberOfTrips) {}
    ...
    inline bool operator<(const WalkingParetoLabel& other) const noexcept {
        return std::tie(numberOfTrips, arrivalTime, walkingDistance)
               < std::tie(other.numberOfTrips, other.arrivalTime, other.walkingDistance);
    }
...
struct MultimodalParetoLabel {
    ...
    inline bool operator<(const MultimodalParetoLabel& other) const noexcept {
        return std::tie(numberOfTrips, arrivalTime, transferTime[0])
               < std::tie(other.numberOfTrips, other.arrivalTime, other.transferTime[0]);
    }
