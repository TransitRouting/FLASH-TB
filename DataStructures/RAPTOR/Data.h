...
class Data {
 ...
 public:
  inline void printInfo() const noexcept {
    ...
    std::cout << "   Max Speed [m/s]:          " << std::setw(12)
              << String::prettyDouble(maxSpeed) << std::endl;
    ...
    std::cout << "   Number of Cells:          " << std::setw(12)
              << numberOfPartitions << std::endl;
  ...
  inline void writeMETISFile(const std::string& fileName,
  ...
  inline void updatePartitionValuesFromFile(const std::string fileName = "",
                                            const bool verbose = true) {
    if (verbose)
    ...
    while (std::getline(partitionFile, line)) {
      ...
      newNumberOfPartitions =
          (newNumberOfPartitions < stopData[index].partition)
              ? stopData[index].partition
              : newNumberOfPartitions;
      ++index;
    }
    if (verbose)
