...
public:
    ...
    inline int getTripOffset(const RouteId route) const noexcept {
        ...
        for (size_t i = 1; i < tripCount; i++) {
            AssertMsg(offset
                          == stopEvents[firstStopEventOfRoute[route] + (i * stopCount)].departureTime
    ...
    inline Data reverseNetwork(Permutation& stopEventPermutation) const noexcept {
        ...
        for (const RouteSegment routeSegment : routeSegments) {
            result.routeSegments.emplace_back(routeSegment.routeId, StopIndex(numberOfStopsInRoute(routeSegment.routeId)
...
public:
    ...
    inline int getPartitionCellOfStop(const StopId stop) { return stopData[stop].partition; }
    ...
    inline void updatePartitionValuesFromFile(const std::string fileName = "", const bool verbose = true) {
        ...
        while (std::getline(partitionFile, line)) {
            ...
            newNumberOfPartitions =
                (newNumberOfPartitions < stopData[index].partition) ? stopData[index].partition : newNumberOfPartitions;
            ++index;
        }
