...
public:
    ...
    public:
        Iterator(const IndirectEdgeRange* const indirectEdgeRange, const VertexIterator& currentVertex)
            : indirectEdgeRange(indirectEdgeRange), currentVertex(currentVertex), undefined(true) {
        ...
        inline bool operator!=(const Iterator& other) const noexcept { return currentVertex != other.currentVertex; }
    ...
    IndirectEdgeRange() : graph(nullptr), vertices(), endVertex(vertices.end()) {}

    IndirectEdgeRange(const Graph& graph) : graph(&graph), vertices(graph.vertices()), endVertex(vertices.end()) {}

    IndirectEdgeRange(const Graph&&) = delete;

    inline Iterator begin() const noexcept { return Iterator(this, vertices.begin()); }
