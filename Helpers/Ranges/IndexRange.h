...
public:
    ...
    IndexRange(const Index& index, const Range& range, const size_t beginIndex, const size_t endIndex)
        : index(&index), range(&range), beginIndex(beginIndex), endIndex(endIndex) {}

    IndexRange(const Index& index, const Range& range, const size_t beginIndex = 0)
        : index(&index), range(&range), beginIndex(beginIndex), endIndex(range.size()) {}

    IndexRange(const Index&&, const Range&, const size_t = 0, const size_t = 0) = delete;
    IndexRange(const Index&, const Range&&, const size_t = 0, const size_t = 0) = delete;

    inline Iterator begin() const noexcept { return Iterator(index, range, beginIndex); }

    inline Iterator end() const noexcept { return Iterator(index, range, endIndex); }
