...
public:
    ...
    DirectEdgeRange() : graph(nullptr) {}

    DirectEdgeRange(const Graph& graph) : graph(&graph), edges(graph.edges()) {}

    DirectEdgeRange(const Graph&&) = delete;

    inline Iterator begin() const noexcept { return Iterator(graph, edges.begin()); }
