...
public:
    ...
    inline bool reachable(const Vertex vertex) const noexcept { return arrivalTime[vertex] < never; }

    inline int getEarliestArrivalTime(const Vertex vertex) const noexcept { return arrivalTime[vertex]; }
        ...
        while (vertex != sourceVertex) {
            ...
            if (label.reachedByTransfer) {
                const int parentDepartureTime =
                    (label.parent == sourceVertex) ? sourceDepartureTime : arrivalTime[label.parent];
    ...
    inline std::vector<Vertex> getPath(const Vertex vertex) noexcept { return journeyToPath(getJourney(vertex)); }
