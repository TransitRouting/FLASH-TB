...
public:
    ...
    inline void setTentativeDistance(const int distance) noexcept { tentativeDistance = distance; }

    inline bool reachable() const noexcept { return intersectingVertex != noVertex; }

    inline bool visited(const Vertex vertex) const noexcept { return timeStamp[vertex] == time; }

    inline int getDistance(const Vertex = noVertex) const noexcept { return tentativeDistance; }

    inline Vertex getIntersectingVertex() const noexcept { return intersectingVertex; }
    ...
    inline std::vector<Vertex> getPackedForwardLeg() const noexcept { return forwardLeg<false>(); }

    inline std::vector<Vertex> getPackedBackwardLeg() const noexcept { return backwardLeg<false>(); }
