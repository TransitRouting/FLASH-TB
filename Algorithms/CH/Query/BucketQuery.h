...
public:
    ...
    inline void setTentativeDistance(const int distance) noexcept { baseQuery.setTentativeDistance(distance); }
    ...
    inline bool visited(const Vertex vertex) const noexcept { return baseQuery.visited(vertex); }

    inline int getDistance(const Vertex = noVertex) const noexcept { return baseQuery.getDistance(); }

    inline const std::vector<int>& getForwardDistance() const noexcept { return distance[FORWARD]; }

    inline int getForwardDistance(const Vertex vertex) const noexcept { return distance[FORWARD][vertex]; }

    inline const std::vector<int>& getBackwardDistance() const noexcept { return distance[BACKWARD]; }

    inline int getBackwardDistance(const Vertex vertex) const noexcept { return distance[BACKWARD][vertex]; }

    inline const std::vector<Vertex>& getForwardPOIs() const noexcept { return reachedPOIs[FORWARD]; }

    inline const std::vector<Vertex>& getBackwardPOIs() const noexcept { return reachedPOIs[BACKWARD]; }

    inline std::vector<Vertex> getReversePath(const Vertex = noVertex) const noexcept {
        return baseQuery.getReversePath();
    }

    inline std::vector<Vertex> getPath(const Vertex = noVertex) const noexcept { return baseQuery.getPath(); }

    inline int getSettleCount() const noexcept { return baseQuery.getSettleCount(); }

    inline int getStallCount() const noexcept { return baseQuery.getStallCount(); }
        ...
        for (const Vertex vertex : baseQuery.template getPOIs<DIRECTION>()) {
            ...
            for (const Edge edge : bucketGraph[DIRECTION].edgesFrom(vertex)) {
                const int newDistance =
                    baseQuery.template getDistanceToPOI<DIRECTION>(vertex) + bucketGraph[DIRECTION].get(Weight, edge);
