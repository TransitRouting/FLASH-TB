...
public:
    ...
    inline void doneContracting() noexcept { std::cout << "Done contracting vertices." << std::endl; }
    ...
    inline void doneWitnessSearch() noexcept { witnessSearchTime += witnessSearchTimer.elapsedMilliseconds(); }
...
private:
    ...
    inline void printStatistics() noexcept {
        ...
        if (contracted > 0) {
            const double chDegree =
                (data->forwardCH.numEdges() + data->backwardCH.numEdges()) / static_cast<double>(contracted);
