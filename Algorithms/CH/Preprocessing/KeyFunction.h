...
public:
    ...
    inline KeyType operator()(const Vertex vertex) noexcept {
        ...
        const int key = ((shortcutWeight * shortcutsAdded) / (inDegree + outDegree))
                        + ((levelWeight * data->level[vertex])) + ((degreeWeight * inDegree * outDegree));
        Assert(key >= 0);
        return key;
    }
        ...
        for (Edge first : data->core.edgesTo(vertex)) {
            ...
            for (Edge second : data->core.edgesFrom(vertex)) {
                ...
                if (witnessSearch->shortcutIsNecessary(
                        from, to, vertex, data->core.get(Weight, first) + data->core.get(Weight, second))) {
                    shortcutsAdded++;
                }
            }
        }
...
public:
    ...
    inline KeyType operator()(const Vertex vertex) noexcept { return permutation[vertex]; }

    template <typename T>
    inline void update(T&) noexcept {}

    inline void initialize(const Data* data, WitnessSearch*) noexcept {
        AssertMsg(permutation.size() == data->numVertices, "Permutation of size " << permutation.size()
...
public:
    PartialKey(const std::vector<bool>& contractable, const size_t minOrderIndex,
        ...
        : data(nullptr), contractable(contractable), keyFunction(keyFunction), minOrderIndex(minOrderIndex) {}
