...
public:
    ...
    Data(GRAPH&& graph, std::vector<int> weight) : numVertices(graph.numVertices()) {
    ...
    Data(GRAPH&& graph, const AttributeNameWrapper<ATTRIBUTE_NAME> weight) : numVertices(graph.numVertices()) {
    ...
    Data(CHCoreGraph&& graph) : numVertices(graph.numVertices()), core(std::move(graph)) {
        forwardCH.addVertices(numVertices);
        backwardCH.addVertices(numVertices);
        std::vector<uint16_t>(numVertices, 0).swap(level);
    }

    inline size_t coreSize() const noexcept { return level.size() - order.size(); }
