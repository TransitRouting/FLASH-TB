...
public:
    ...
    using Type =
        Builder<Profiler, WitnessSearch, KeyFunction, StopCriterion, BuildQLinear, BreakKeyTiesById, SortShortcuts>;
...
public:
    ...
    inline size_t numberOfUncontractedVertices() const noexcept { return Q.size(); }

    inline const CHCoreGraph& getCore() const noexcept { return data.core; }

    inline CHCoreGraph& getCore() noexcept { return data.core; }

    inline const std::vector<Vertex>& getOrder() const noexcept { return data.order; }
...
private:
    ...
    inline KeyType getKey(const Vertex vertex) noexcept { return keyFunction(vertex); }
