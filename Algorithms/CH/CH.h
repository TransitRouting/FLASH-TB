...
public:
    ...
    CH(Data&& data) : CH(std::move(data.forwardCH), std::move(data.backwardCH)) {}
    ...
    CH(const std::string& fileName, const std::string& separator = ".") { readBinary(fileName, separator); }
    ...
    inline size_t numEdges() const noexcept { return forward.numEdges() + backward.numEdges(); }
    ...
    inline IndirectEdgeRange<CH> edgesWithFromVertex() const noexcept { return IndirectEdgeRange<CH>(*this); }
