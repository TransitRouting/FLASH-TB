...
public:
    struct ShortcutInfo {
        ShortcutInfo(const StopId origin, const StopId destination, const int length, const StopEventId target,
                     const bool isProper)
            : origin(origin), destination(destination), length(length), isProper(isProper) {
    ...
    struct Shortcut {
        Shortcut(const ShortcutInfo& info)
            : origin(info.origin), destination(info.destination), travelTime(info.length) {}
    ...
    struct InitialTransferLabel : public ExternalKHeapElement {
        InitialTransferLabel(const int transferDistance = INFTY, const int transferCost = INFTY)
            : transferDistance(transferDistance), transferCost(transferCost) {}
    ...
    struct OneTripLabel : public ExternalKHeapElement {
        ...
        inline bool dominates(const LABEL& other) const noexcept {
            return arrivalTime <= other.arrivalTime
                   && getDominanceWalkingDistance() <= other.getDominanceWalkingDistance();
        }
        ...
        inline bool isCandidate() const noexcept { return shortcutOrigin != noStop; }

        inline void witnessify() noexcept { shortcutOrigin = noStop; }

        inline int getKey() const noexcept { return arrivalTime; }

        inline KeyType getQueueKey() const noexcept { return std::make_tuple(arrivalTime, walkingDistance); }
    ...
    struct TwoTripsLabel : public ExternalKHeapElement {
        ...
        inline bool isCandidate() const noexcept { return finalStopEvent != noStopEvent; }

        inline void witnessify() noexcept { finalStopEvent = noStopEvent; }
    ...
    public:
        ...
        template <typename FUNCTION = NO_OPERATION>
        inline bool mergeWitness(const Label& newLabel,
                                 const FUNCTION& processRemovedHeapLabel = NoOperation) noexcept {
        ...
        mergeTransitiveWitness(const Label& newLabel, const FUNCTION& processRemovedHeapLabel = NoOperation) noexcept {
    ...
    struct RouteLabel {
        ...
        inline const StopEvent* parentStopEvent() const noexcept { return trip + parentStopIndex; }
    ...
    struct DepartureLabel {
        ...
        inline bool operator<(const DepartureLabel& other) const noexcept {
            return (departureTime > other.departureTime)
                   || ((departureTime == other.departureTime) && (route.routeId < other.route.routeId));
        }
    };
        ...
        for (const ConsolidatedDepartureLabel& label : collectDepartures(minTime, maxTime)) {
            ...
            for (const Shortcut& shortcut : shortcuts) {
                if (!shortcutGraph.hasEdge(shortcut.origin, shortcut.destination)) {
                    ...
                    AssertMsg(
                        shortcutGraph.get(TravelTime, shortcutGraph.findEdge(shortcut.origin, shortcut.destination))
                            == shortcut.travelTime,
...
private:
    ...
    inline void collectRoutes1(const std::vector<RouteSegment>& routes) noexcept {
        for (const RouteSegment& route : routes) {
            ...
            AssertMsg(data.lastTripOfRoute(route.routeId)[route.stopIndex].departureTime
                          >= initialArrivalTime(data.stopOfRouteSegment(route)),
        ...
        for (const RouteId route : routesServingUpdatedStops.getKeys()) {
            ...
            while (tripIterator.hasFurtherStops()) {
                ...
                if (tripIterator.hasEarlierTrip()
                    && (tripIterator.previousDepartureTime() >= initialArrivalTime(tripIterator.stop()))) {
    ...
    inline void arrivalByRoute2(const StopId stop, TwoTripsLabel& label, const RouteLabel& routeLabel,
        ...
        const bool isCandidate = routeLabel.isCandidate() && (routeLabel.shortcutOrigin != parentStop)
                                 && !shortcutGraph.hasEdge(routeLabel.shortcutOrigin, parentStop);
        if (isCandidate) {
    ...
    inline bool updateOneTripBagTransitiveWitness(const Vertex vertex, const OneTripLabel& newLabel) noexcept {
        ...
        std::tie(merged, removedHeapLabels) =
            oneTripBags[vertex].mergeTransitiveWitness(newLabel, [&](const OneTripLabel& removedLabel) {
                if (removedLabel.isCandidate()) shortcutCandidatesInQueue--;
            });
