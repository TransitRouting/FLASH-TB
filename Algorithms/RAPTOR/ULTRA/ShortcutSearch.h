...
private:
    ...
    inline void finalDijkstra() noexcept {
        ...
        AssertMsg(shortcutDestinationCandidates.empty(), "There are still shortcut destination candidates left ("
    ...
    inline void arrivalByRoute2(const StopId stop, const int arrivalTime, const StopId parent) noexcept {
        // Mark journey as candidate or witness
        if (data.isStop(oneTripTransferParent[parent]) && (oneTripTransferParent[parent] != parent)
