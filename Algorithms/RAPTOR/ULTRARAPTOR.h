...
public:
    ...
    inline int getEarliestArrivalTime() const noexcept { return getEarliestArrivalTime(targetVertex); }
    ...
    inline int getWalkingArrivalTime() const noexcept { return sourceDepartureTime + initialTransfers.getDistance(); }
    ...
    inline int getWalkingTravelTime() const noexcept { return initialTransfers.getDistance(); }

    inline int getWalkingTravelTime(const Vertex vertex) const noexcept { return initialTransfers.getDistance(vertex); }

    inline int getDirectTransferTime() const noexcept { return initialTransfers.getDistance(); }
    ...
    inline std::vector<Vertex> getPath() const noexcept { return getPath(targetVertex); }
    ...
    inline int getArrivalTime(const Vertex vertex, const size_t numberOfTrips) const noexcept {
        ...
        if constexpr (SeparateRouteAndTransferEntries) {
            if ((round + 1 < rounds.size())
                && (rounds[round + 1][target].arrivalTime < rounds[round][target].arrivalTime))
                round++;
        }
...
private:
    ...
    inline void startNewRound() noexcept { rounds.emplace_back(data.numberOfStops() + 1); }
