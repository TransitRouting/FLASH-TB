...
public:
    ...
    inline const std::vector<ArrivalLabel>& getAnchorLabels() const noexcept { return anchorLabels; }
...
private:
    ...
    inline void scanRoutes() noexcept {
        ...
        for (const RouteId route : routesServingUpdatedStops.getKeys()) {
            ...
            while (stopIndex < tripSize - 1) {
                while (
                    (trip > firstTrip)
                    && ((trip - tripSize + stopIndex)->departureTime >= previousRound()[stop].arrivalTimeByTransfer)) {
                    trip -= tripSize;
    ...
    inline void arrivalByRoute(const StopId stop, const int time) noexcept {
        AssertMsg(data.isStop(stop), "Stop " << stop << " is out of range!");
        if ((currentRound()[targetStop].arrivalTimeByTransfer - sourceDepartureTime) * arrivalSlack
