...
private:
    ...
    struct BestLabel {
        ...
        BestLabel(const int arrivalTime) : arrivalTime(arrivalTime), transferTime{0} {}
    ...
    struct RouteLabel {
        ...
        inline int getArrivalTime(const StopIndex stopIndex) const noexcept { return trip[stopIndex].arrivalTime; }

        inline int parentDepartureTime() const noexcept { return trip[parentStop].departureTime; }
...
public:
    ...
    inline void verify(const double arrivalSlack, const double tripSlack, const int departureTime) const noexcept {
        ...
        for (const ArrivalLabel& anchorLabel : anchorLabels) {
            Ensure(isContained(anchorLabel), "Anchor label with arrival time " << anchorLabel.arrivalTime << " and "
    ...
    inline std::vector<ParetoLabel> getResults() const noexcept { return getResults(targetStop); }
...
private:
    ...
    inline void collectRoutesServingUpdatedStops() noexcept {
        for (const StopId stop : stopsUpdatedByTransfer) {
            for (const RouteSegment& route : data.raptorData.routesContainingStop(stop)) {
                ...
                AssertMsg(data.raptorData.stopIds[data.raptorData.firstStopIdOfRoute[route.routeId] + route.stopIndex]
                              == stop,
    ...
    inline void startNewRound() noexcept { rounds.emplace_back(data.raptorData.numberOfStops() + 1); }

    inline size_t currentNumberOfTrips() const noexcept { return (rounds.size() - 1) / 2; }
