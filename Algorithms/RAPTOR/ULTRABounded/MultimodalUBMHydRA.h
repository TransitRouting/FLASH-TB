...
private:
    ...
    struct RouteLabel {
        ...
        RouteLabel(const StopEvent* trip, const Label& label, const StopIndex parentStop, const size_t parentIndex,
            ...
            : trip(trip), parentStop(parentStop), parentIndex(parentIndex), lastExitIndex(lastExitIndex) {
            std::copy(std::begin(label.transferTime), std::end(label.transferTime), std::begin(transferTime));
        }
...
private:
    ...
    inline void startNewRound() noexcept { rounds.emplace_back(raptorData.numberOfStops() + 1); }
