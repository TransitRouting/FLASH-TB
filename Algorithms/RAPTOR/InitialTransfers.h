...
public:
    ...
    inline int getDistance(const Vertex = noVertex) const noexcept { return targetDistance; }
...
public:
    ...
    inline const std::vector<Vertex>& getForwardPOIs() const noexcept { return reachedPOIs[FORWARD].getValues(); }

    inline const std::vector<Vertex>& getBackwardPOIs() const noexcept { return reachedPOIs[BACKWARD].getValues(); }
    ...
    inline void evaluateTransfers(TRANSFERS& transfers, const int overhead) noexcept {
        for (const Vertex vertex : transfers.getForwardPOIs()) {
            ...
            distance[FORWARD][vertex] =
                std::min(distance[FORWARD][vertex], transfers.getForwardDistance(vertex) + overhead);
        }
        for (const Vertex vertex : transfers.getBackwardPOIs()) {
            reachedPOIs[BACKWARD].insert(vertex);
            distance[BACKWARD][vertex] =
                std::min(distance[BACKWARD][vertex], transfers.getBackwardDistance(vertex) + overhead);
        }
