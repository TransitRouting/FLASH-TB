...
public:
    BackwardPruningQuery(const Data& data,
        ...
        for (StopEventId stopEvent(0); stopEvent < data.numberOfStopEvents(); stopEvent++) {
            properArrivalTimes[stopEvent] = data.arrivalEvents[stopEvent].arrivalTime
                                            - data.raptorData.minTransferTime(data.getStopOfStopEvent(stopEvent));
        }
    }
    ...
    inline void run(const Vertex source, const int originalDepartureTime, const Vertex target,
        ...
        for (const RAPTOR::ArrivalLabel& label : forwardPruningQuery.getAnchorLabels()) {
            ...
            roundOffset =
                maxTrips - std::min(size_t(std::ceil(label.numberOfTrips * tripSlack)), lastNumberOfTrips - 1);
    ...
    inline int getDepartureTime(const size_t numTrips) const noexcept { return departureTimes[numTrips]; }
