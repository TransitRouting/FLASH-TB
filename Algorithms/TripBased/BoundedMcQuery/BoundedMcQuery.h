...
private:
    ...
    struct TargetLabel {
        TargetLabel(const int arrivalTime = never, int walkingDistance = INFTY, const u_int32_t parent = -1)
            : arrivalTime(arrivalTime), walkingDistance(walkingDistance), parent(parent) {}
...
private:
    ...
    inline size_t currentNumberOfTrips() const noexcept { return targetBags.size() - 1; }
    ...
    inline void enqueue(const TripId trip, const StopIndex index, const int walkingDistance) noexcept {
        ...
        if (backwardPruningQuery.getReachedIndex(info.reverseTrip, maxTrips - currentNumberOfTrips())
            > reverseStopIndex)
            return;
        const StopEventId end =
            walkingDistanceData.getScanEnd(StopEventId(stopEvent + 1), info.tripEnd, walkingDistance);
    ...
    inline void enqueue(const Edge edge, int walkingDistance, const u_int32_t parent) noexcept {
        ...
        if (backwardPruningQuery.getReachedIndex(label.reverseTrip, maxTrips - currentNumberOfTrips())
            > label.reverseStopIndex)
            return;
        const StopEventId end =
            walkingDistanceData.getScanEnd(StopEventId(label.stopEvent + 1), label.tripEnd, walkingDistance);
