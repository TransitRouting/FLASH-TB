...
private:
    ...
    inline void enqueue(const Edge edge, const size_t parent) noexcept {
        ...
        const long distToTarget =
        geoDistanceInCM(data.raptorData.stopData[data.getStopOfStopEvent(label.stopEvent)].coordinates,
        data.raptorData.stopData[lastTarget].coordinates); if
        (data.raptorData.stopEvents[label.stopEvent].departureTime + distToTarget /
        (100.0 * data.raptorData.maxSpeed) >= minArrivalTime) {
