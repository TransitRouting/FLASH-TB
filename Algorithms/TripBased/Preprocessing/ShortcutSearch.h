...
class ShortcutSearch {
  ...
  inline std::vector<ConsolidatedDepartureLabel> collectDepartures(
    ...
    for (const RouteId route : data.routes()) {
      ...
      for (size_t stopIndex = 0; stopIndex + 1 < tripSize; stopIndex++) {
        if (directTransferArrivalLabels[stops[stopIndex]].arrivalTime >
            minimalTransferTime)
          continue;
        minimalTransferTime =
        ...
        for (const RAPTOR::StopEvent* trip = data.firstTripOfRoute(route);
          ...
          const int departureTime =
              trip[stopIndex].departureTime - minimalTransferTime;
 ...
 private:
  inline void clear() noexcept {
    ...
    std::vector<StopEventId>(data.transferGraph.numVertices(), noStopEvent)
  ...
  inline void intermediateDijkstra() noexcept {
    ...
    while (!oneTripQueue.empty()) {
      ...
      const Vertex currentVertex =
          Vertex(currentLabel - &(oneTripArrivalLabels[0]));
      for (Edge edge : data.transferGraph.edgesFrom(currentVertex)) {
        const Vertex neighborVertex = data.transferGraph.get(ToVertex, edge);
        const int newArrivalTime = currentLabel->arrivalTime +
                                   data.transferGraph.get(TravelTime, edge);
        ...
        // iterations
                 ...
                 oneTripArrivalLabels[neighborVertex].arrivalTime) {
          const bool isCandidate =
              oneTripTransferParent[currentVertex] != noStopEvent;
          const bool isFromPreviousIteration =
              oneTripTimestamps[neighborVertex] != timestamp;
  ...
  inline void finalDijkstra() noexcept {
    ...
    while (!twoTripsQueue.empty()) {
      ...
      const Vertex currentVertex =
          Vertex(currentLabel - &(twoTripsArrivalLabels[0]));
      for (Edge edge : data.transferGraph.edgesFrom(currentVertex)) {
        const Vertex neighborVertex = data.transferGraph.get(ToVertex, edge);
        const int newArrivalTime = currentLabel->arrivalTime +
                                   data.transferGraph.get(TravelTime, edge);
        if (newArrivalTime <
            twoTripsArrivalLabels[neighborVertex].arrivalTime) {
      ...
      if (data.isStop(currentVertex)) {
        ...
        if (data.isStop(routeParent)) {
          ...
          // shortcut
          const StopEventId routeParentEvent =
              twoTripsRouteParentEvent[currentVertex];
          const StopEventId transferParentEvent =
              oneTripTransferParent[routeParent];
          const int walkingDistance =
              oneTripArrivalLabels[routeParent].arrivalTime -
              data.stopEvents[transferParentEvent].arrivalTime;
          if constexpr (IgnoreIsolatedCandidates) {
            if (directTransferArrivalLabels[currentVertex].arrivalTime <
                never) {
          ...
          AssertMsg(shortcutDestinationCandidates.contains(routeParentEvent),
                    "Vertex "
                        << currentVertex << " has route parent "
                        << routeParentEvent
          ...
          for (const StopId obsoleteCandidate :
               shortcutDestinationCandidates[routeParentEvent]) {
    ...
    if (data.isStop(twoTripsRouteParent[vertex])) {
      ...
      AssertMsg(shortcutDestinationCandidates.contains(routeParentEvent),
                "Vertex " << vertex << " has route parent " << routeParentEvent
                          << " but the route parent does not know about this!");
      AssertMsg(shortcutDestinationCandidates[routeParentEvent].contains(
                    StopId(vertex)),
                "Vertex " << vertex
                          << " is not contained in "
  ...
  inline bool isShortcutCandidate(const Vertex vertex) noexcept {
    return oneTripArrivalLabels[vertex].isOnHeap() &&
           (oneTripTransferParent[vertex] != noStopEvent);
  }
