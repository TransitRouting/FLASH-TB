            ...
            for (size_t stopIndex = 0; stopIndex + 1 < tripSize; stopIndex++) {
                for (const RAPTOR::StopEvent* trip = data.firstTripOfRoute(route); trip <= data.lastTripOfRoute(route);
                    ...
                    const int departureTime =
                        trip[stopIndex].departureTime - directTransferArrivalLabels[stops[stopIndex]].arrivalTime;
...
private:
    ...
    inline void relaxFinalTransfers() noexcept {
        ...
        for (const StopEventId destinationEvent : shortcutDestinationCandidates.getKeys()) {
            ...
            const int walkingDistance =
                oneTripArrivalLabels[destinationStop].arrivalTime - data.stopEvents[originEvent].arrivalTime;
    ...
    inline void arrivalByEdge2(const StopId stop, const int arrivalTime, const Vertex parent) noexcept {
        ...
        if (twoTripsRouteParent[stop] != noStop) {
            ...
            AssertMsg(shortcutDestinationCandidates[routeParentEvent].targetStops.contains(stop),
                      "Stop " << stop << " is not contained in shortcutDestinationCandidates List of "
                              << routeParentEvent << "!");
