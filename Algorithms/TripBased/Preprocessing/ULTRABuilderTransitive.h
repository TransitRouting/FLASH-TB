...
public:
    ULTRABuilderTransitive(const Data& data) : data(data) { stopEventGraph.addVertices(data.numberOfStopEvents()); }

    void computeShortcuts(const ThreadPinning& threadPinning, const int minDepartureTime = -never,
...
#pragma omp critical
        ...
        if (!shortcuts.empty()) {
            ...
            stopEventGraph.addEdge(Vertex(shortcuts[0].origin), Vertex(shortcuts[0].destination))
                ...
                .set(ARCFlag, emptyFlags);
            for (size_t i = 1; i < shortcuts.size(); i++) {
                if ((shortcuts[i].origin == shortcuts[i - 1].origin)
                    && (shortcuts[i].destination == shortcuts[i - 1].destination))
                    continue;
                std::vector<bool> emptyFlags(data.getNumberOfPartitionCells(), false);
    ...
    inline const DynamicTransferGraphWithARCFlag& getStopEventGraph() const noexcept { return stopEventGraph; }

    inline DynamicTransferGraphWithARCFlag& getStopEventGraph() noexcept { return stopEventGraph; }
