...
class ULTRABuilderTransitive {
 ...
 public:
  ULTRABuilderTransitive(const Data& data) : data(data) {
...
#pragma omp critical
      {
        const std::vector<Shortcut>& localShortcuts =
            shortcutSearch.getShortcuts();
  ...
  inline const DynamicTransferGraphWithARCFlag& getStopEventGraph()
      const noexcept {
