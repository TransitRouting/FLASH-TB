...
public:
    struct ShortcutInfo {
        ShortcutInfo(const StopEventId origin, const int length, const StopEventId target, const bool isProper)
            : origin(origin), length(length), isProper(isProper) {
    ...
    struct OneTripLabel : public ExternalKHeapElement {
        ...
        inline bool isCandidate() const noexcept { return shortcutOrigin != noStopEvent; }

        inline void witnessify() noexcept { shortcutOrigin = noStopEvent; }
    ...
    struct RouteLabel {
        ...
        inline const RAPTOR::StopEvent* parentStopEvent() const noexcept { return trip + parentStopIndex; }
...
public:
    ...
    inline const std::vector<Shortcut>& getShortcuts() noexcept { return shortcuts; }
